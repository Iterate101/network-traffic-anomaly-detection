...
- 但真实多攻击实验发现，不同 mask ratio 会改变 Recall 和 F1 的取舍，所以掩码重构不是无条件提升，这正是消融实验的意义。
...
- 本次正式实验混合了 DDoS、PortScan 和 WebAttack，不再只是单一 PortScan。
- RandomForest 和 MLP 接近满分，说明 CIC-IDS2017 的监督表格特征很强。
- Vanilla AE 的 F1 是 0.7690，ROC-AUC 是 0.8840，说明普通重构误差异常检测本身有效。
- Masked Transformer AE 的 Recall 是 0.9930，是自监督模型里最高的，但 F1 是 0.6709，说明它更偏向少漏报，但误报增加。
- `attack_type_metrics.csv` 可以单独展示 DDoS、PortScan、WebAttack 的检测效果，说明我们不是只看总体平均。
